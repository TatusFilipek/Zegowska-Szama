(function () {
    ...
    async function reloadOrders() {
        ...
        try {
            ...
            const uniqueOrders = await res.json();
            ...
            tbody.innerHTML = uniqueOrders.map(o => {
                ...
                switch(currentStatus) {
                    case 1:
                        ...
                        buttonHtml = `<button class="btn btn-sm fw-semibold text-white px-3 py-1 rounded-2" onclick="event.stopPropagation(); progressOrderStatus(${o.id})" style="background-color: #5a8e7a; border: none;">Start Preparing ➔</button>`;
                        break;
                    case 2:
                        statusText = `<span class="badge bg-info text-dark px-2 py-1">Preparing (2)</span>`;
                        buttonHtml = `<button class="btn btn-sm fw-semibold text-white px-3 py-1 rounded-2" onclick="event.stopPropagation(); progressOrderStatus(${o.id})" style="background-color: #2e3d52; border: none;">Mark as Ready ➔</button>`;
                        break;
                    case 3:
                        statusText = `<span class="badge bg-success text-white px-2 py-1">Ready to Collect (3)</span>`;
                        buttonHtml = `<button class="btn btn-sm fw-semibold text-white px-3 py-1 rounded-2" onclick="event.stopPropagation(); archiveOrderStatus(${o.id})" style="background-color: #3b4257; border: none;">Archive Order ✓</button>`;
                        break;
                    default:
                ...
                if (!o.items || o.items.length === 0) {
                    itemsListHtml = '<li class="text-muted">Brak pozycji w zamówieniu (lub tabele w bazie danych różnią się strukturą).</li>';
                } else {
                    itemsListHtml = o.items.map(item => {
                        const cost = parseInt(item.price_cents, 10) * parseInt(item.quantity, 10);
                        totalCents += cost;
                        const labelType = item.item_type === 'offer' ? '<span class="badge bg-secondary me-1">Oferta</span>' : '<span class="badge bg-light text-dark border me-1">Produkt</span>';
                        return `
                            <li class="d-flex justify-content-between align-items-center py-1 border-bottom" style="border-color: #e2e8f0 !important;">
                                <span>${labelType} <strong>${item.name}</strong> x${item.quantity}</span>
                                <span class="text-muted">${formatPrice(cost)}</span>
                            </li>
                        `;
                    }).join('');
                }
                ...
                return `
                    <tr onclick="toggleOrderDetails(${o.id})" style="color: #2e3d52; vertical-align: middle; cursor: pointer;" title="Kliknij, aby zobaczyć szczegóły">
                        <td>${o.name}</td>
                        ...
                        <td>
                            <div class="d-flex gap-2 align-items-center">
                                ${buttonHtml}
                                <button class="btn btn-sm btn-light border fw-semibold px-2" style="color: #4a586e;">Rozwiń ➔</button>
                            </div>
                        </td>
                    </tr>
                    <tr id="details-${o.id}" class="d-none" style="background-color: #f8fafc;">
                        <td colspan="4" class="p-3">
                            <div class="card card-body border-0 shadow-sm rounded-3 p-3 mx-2" style="background-color: #ffffff; color: #2e3d52;">
                                <h6 class="fw-bold mb-3" style="color: #3b4257;">Zawartość zamówienia #${String(o.id).padStart(2, '0')}:</h6>
                                <ul class="list-unstyled mb-3">
                                    ${itemsListHtml}
                                </ul>
                                <div class="d-flex justify-content-between align-items-center pt-2" style="border-top: 2px solid #5a8e7a;">
                                    <span class="fw-bold fs-5" style="color: #2e3d52;">Suma całkowita:</span>
                                    <span class="fw-bold fs-5 text-success" style="color: #5a8e7a !important;">${formatPrice(totalCents)}</span>
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            console.error(e);
            tbody.innerHTML = `<tr><td colspan="4" class="text-danger text-center">Błąd synchronizacji zamówień. Upewnij się, że struktura bazy danych odpowiada zapytaniu.</td></tr>`;
        }
    }
    ...
    window.closeProductModal = function () { document.getElementById('product-modal').classList.remove('show'); };
    window.closeProductModalOnOutsideClick = function (event) { if (event.target.id === 'product-modal') closeProductModal(); };
    ...
    document.getElementById('product-form').addEventListener('submit', async function (e) {
        ...
        } catch (e) { alert('Wystąpił błąd zapisu produktu.'); }
    });
    ...
    async function fetchProductsToCache() {
        ...
        } catch (e) { console.error("Błąd ładowania produktów", e); }
    }
    ...
    async function reloadOffers() {
        ...
        try {
            ...
            if (globalProductsCache.length === 0) await fetchProductsToCache();
        ...
        } catch (e) { tbody.innerHTML = `<tr><td colspan="4" class="text-danger text-center">Błąd pobierania bazy ofert.</td></tr>`; }
    }
    ...
    function renderOfferProductsInputs(selectedProducts = []) {
        ...
        selectedProducts.forEach(p => { activeQuantities[p.product_id] = p.quantity; });
        container.innerHTML = globalProductsCache.map(p => {
            ...
            return `
                <div class="d-flex align-items-center justify-content-between py-1 border-bottom" style="border-color: #e2e8f0 !important;">
                    ...
                    <input type="number" class="form-content offer-product-qty-input" data-product-id="${p.id}" value="${currentQty}" min="0" style="width: 70px; text-align: center; border: 1px solid #cbd5e1; border-radius: 4px; font-weight: 600; color: #334155;">
                </div>
            `;
        }).join('');
    }
    ...
    window.closeOfferModal = function () { document.getElementById('offer-modal').classList.remove('show'); };
    window.closeOfferModalOnOutsideClick = function (event) { if (event.target.id === 'offer-modal') closeOfferModal(); };
    ...
    document.getElementById('offer-form').addEventListener('submit', async function (e) {
        ...
        document.querySelectorAll('.offer-product-qty-input').forEach(input => {
            ...
            if (qty > 0) {
                items.push({ product_id: parseInt(input.getAttribute('data-product-id'), 10), quantity: qty });
            }
        });
        const payload = {
            id: idVal ? parseInt(idVal, 10) : null,
        ...
        } catch (e) { alert('Wystąpił błąd zapisu oferty.'); }
    });
    ...
    async function reloadUsers() {
        ...
        } catch (e) { tbody.innerHTML = `<tr><td colspan="4" class="text-danger text-center">Błąd pobierania bazy użytkowników.</td></tr>`; }
    }
    ...
    window.promptChangeRole = async function(userId) {
        ...
        } catch (e) { alert('Błąd modyfikacji roli użytkownika.'); }
    };
    ...
    document.getElementById('mail-form').addEventListener('submit', async function (e) {
        ...
        } catch (e) { alert('Wystąpił błąd podczas wysyłania ogłoszenia.'); }
    });
