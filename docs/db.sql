...
CREATE TABLE IF NOT EXISTS `products` (
	...
	CONSTRAINT `constraint_discount` CHECK discount_percent BETWEEN 0 AND 100,
	CONSTRAINT `constraint_price_cents` CHECK price_cents > 0
);
CREATE TABLE IF NOT EXISTS `orders` (
	`id` int AUTO_INCREMENT NOT NULL,
	`user_id` int NOT NULL,
	`status` int NOT NULL COMMENT '0 - pending
1 - processing
2 - completed
3 - paid & collected',
	`created_at` datetime NOT NULL,
...
CREATE TABLE IF NOT EXISTS `order_items` (
	...
	PRIMARY KEY (`id`),
	CONSTRAINT `constraint_quantity` CHECK quantity > 0,
	CONSTRAINT `constraint_unit_price_snapshot` CHECK unit_price_snapshot > 0
);
